...
write_filename = "final_webstaurantstore_" + date_now + ".csv"
...
def get_product_links(page):
	url = f'https://www.webstaurantstore.com/specials.html?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('div', id='product_listing').find_all('div', class_='product-box-container'):
		urls.append('https://www.webstaurantstore.com'+item.find('a')['href'])

def get_product_info(link):
	html = get_zenrows_html(link)
	soup = BeautifulSoup(html, 'html.parser')

	upc = soup.find('span', class_='product__stat-desc').text
	if not upc.isdigit():
		return
