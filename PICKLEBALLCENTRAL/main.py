...
write_filename = "final_pickleballcentral_" + date_now + ".csv"
...
def get_product_links(page):
	url = f'https://pickleballcentral.com/pickleball-specials/?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('ul', class_='productGrid').find_all('li', class_='product'):
...
def get_product_info(link):
	...
	for script in soup.find_all('script', type='text/javascript'):
		if 'var BCData = ' in script.get_text():
			data = script.get_text().replace('var BCData = ','').replace(';','')
			data = json.loads(data)

			upc = data['product_attributes']['upc']
			if len(upc) == 11:
				upc = '0' + upc
			price = data['product_attributes']['price']['without_tax']['value']
