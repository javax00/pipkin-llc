...
write_filename = "final_holabirdsports_" + date_now + ".csv"
...
def get_product_links(page):
	global urls
	# url = f'https://www.holabirdsports.com/collections/extra-savings?page={page}'
	url = f'https://www.holabirdsports.com/collections/25-off-all-thorlo-socks?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')
	
	products = soup.find_all('script', {'type': 'application/ld+json'})[1]
	products = json.loads(products.text)
	for product in products['itemListElement']:
		urls.append('https://www.holabirdsports.com' + product['url'].replace('\\', ''))
