...
write_filename = "final_fairharbor_" + date_now + ".csv"
...
def get_product_links(page):
	url = f'https://www.fairharborclothing.com/collections/end-of-season-sale?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('div', class_='product-listing__list').find_all('div', class_='product-listing__list-item'):
		for var in item.find('div', class_='product-card__option-values-list').find_all('a'):
			urls.append('https://www.fairharborclothing.com'+var['href'].split('?')[0])
