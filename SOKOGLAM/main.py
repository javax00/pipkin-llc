...
write_filename = "final_sokoglam_" + date_now + ".csv"
...
def get_product_links(page):
	url = f'https://sokoglam.com/collections/soko-steals?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('ul', id='product-grid').find_all('li', class_='grid__item'):
		try:
			urls.append('https://sokoglam.com'+item.find_all('a')[1]['href'])
		except:
			continue
