...
write_filename = "final_thesak_" + date_now + ".csv"
...
def get_product_links(page):
	...
	while True:
		try:
			url = f'https://www.thesak.com/collections/sak-sale?page={page}'
			html = get_zenrows_html(url, 'div.selector-wrapper__scrollbar')
			soup = BeautifulSoup(html, 'html.parser')
			soup.find('div', id='CollectionLoop')
			break
		except Exception as e:
			print(f"Try again {retry}...")
			if retry == 3:
				return
			retry += 1

	tot = 0
	for item in soup.find('div', id='CollectionLoop').find_all('div', class_='product-item'):
		for i in item.find('div', class_='selector-wrapper__scrollbar').find_all('tooltip-component'):
			url = 'https://www.thesak.com'+i['data-sibling-link']
			if url not in urls:
				urls.append(url)
				tot += 1
	print(f"Page {page} | Found {tot} product links")
