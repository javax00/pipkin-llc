...
write_filename = "final_harborfreight_" + date_now + ".csv"
...
def get_product_links(page):
	# url = f'https://www.harborfreight.com/merchandising-promotions/clearance.html?current={page}'
	url = f'https://www.harborfreight.com/search?q=instant+savings+collection&current={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find_all('li', class_='grid__item--dQmE-d'):
		urls.append('https://www.harborfreight.com'+item.find('a')['href'])

def get_product_info(link):
	html = get_zenrows_html(link)
	soup = BeautifulSoup(html, 'html.parser')

	upc = soup.find_all('a', {'data-gtm-event': 'e_pdpInteraction'})[-1].text.split(' ')[-1]
	if upc.isdigit():
