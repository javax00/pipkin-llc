...
write_filename = "final_happyplanner_" + date_now + ".csv"
...
def get_product_links(page):
	global urls
	url = f'https://thehappyplanner.com/collections/sale?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')
	for product in soup.find('div', class_='collection-grid').find_all('div', class_='grid-product'):
		with counter_lock:
			urls.append('https://thehappyplanner.com/collections/sale/products/'+product.get('data-product-handle'))
...
if __name__ == "__main__":
	url = 'https://thehappyplanner.com/collections/sale'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')
	total_pages = math.ceil(int(soup.find('div', class_='collection-filter__item--count').text.strip().split(' ')[0])/48)
