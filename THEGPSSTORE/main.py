...
write_filename = "final_thegpsstore_" + date_now + ".csv"
...
def get_product_links(page):
	global urls
	url = f'https://www.thegpsstore.com/Specials-Hot-Deals?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')
	
	for link in soup.find_all('a', class_='product-permalink'):
		urls.append('https://www.thegpsstore.com'+link.get('href'))

def get_product_info(link):
	html = get_zenrows_html(link)
	soup = BeautifulSoup(html, 'html.parser')

	raw_data = soup.find('script', type='application/ld+json').get_text()
	data_json = json.loads(remove_description(raw_data))

	upc = data_json['gtin13'].strip()
	if len(upc) == 13:
		upc = upc[1:]
