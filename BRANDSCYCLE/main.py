...
write_filename = "final_brandscycle_" + date_now + ".csv"
...
def get_product_links(page):
	url = f'https://www.brandscycle.com/product-list/deals-pg859/?startrow={page}&maxItems=60&rb_av=instore'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('div', class_='seSearchProductWrapper').find_all('div', class_='seProduct'):
		urls.append('https://www.brandscycle.com'+item.find('a')['href'])
...
if __name__ == "__main__":
	...
	with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
		futures = [executor.submit(get_product_links, i*60) for i in range(total_pages)]
