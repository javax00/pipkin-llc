...
write_filename = "final_iherb_" + date_now + ".csv"
...
def get_product_links(page):
	url = f'https://www.iherb.com/specials?=0&p={page}&isAjax=true'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('div', class_='products').find_all('div', class_='product-cell-container'):
		url = item.find('a', class_='absolute-link product-link')['href']
		urls.append(url)
...
def get_product_info(link):
	...
	data = soup.find_all('script', type='application/ld+json')[1].get_text()
	data = json.loads(data)

	upc = data['gtin12']
	price = data['offers']['priceSpecification'][0]['price']
