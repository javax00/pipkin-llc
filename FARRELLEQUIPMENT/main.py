...
write_filename = "final_farrellequipment_" + date_now + ".csv"
...
def get_product_links(page):
	global urls
	url = f'https://farrellequipment.com/closeout-deals/?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')
	
	products = soup.find('ul', class_='productGrid').find_all('li', class_='product')
...
def get_product_info(link):
	...
	for script in soup.find_all('script', type='text/javascript'):
		if 'var BCData = ' in script.get_text():
			json_data = json.loads(script.get_text().replace('var BCData = ', '').strip()[:-1])
			price = json_data['product_attributes']['price']['sale_price_without_tax']['value']
