...
def get_product_links(page):
	url = f'https://shop.5undergolf.com/collections/sales-items?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('ul', id='product-grid').find_all('li', class_='grid__item'):
		urls.append('https://shop.5undergolf.com'+item.find('a')['href'])
...
if __name__ == "__main__":
	...
	total_pages = math.ceil(int(get_pages)/25)
