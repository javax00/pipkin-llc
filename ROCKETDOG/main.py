...
write_filename = "final_rocketdog_" + date_now + ".csv"
...
def get_product_links(page):
	# url = f'https://www.rocketdog.com/collections/all?page={page}'
	url = f'https://www.rocketdog.com/collections/all-sneakers?page={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('ul', id='product-grid').find_all('li', class_='grid__item'):
		urls.append('https://www.rocketdog.com'+item.find('a')['href'])
	...
	for variant in data['product']['variants']:
		...
		with counter_lock:
			with open(write_csv, 'a', newline='', encoding='utf-8') as f:
				writer = csv.writer(f)
				writer.writerow([upc, price, title, promo, source_link])

if __name__ == "__main__":
	get_pages = input('Enter total pages: ')

	total_pages = math.ceil(int(get_pages)/88)
