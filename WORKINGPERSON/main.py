...
write_filename = "final_workingperson_" + date_now + ".csv"
...
def get_product_links(page):
	url = f'https://workingperson.com/lp/clearance.html?p={page}'
	html = get_zenrows_html(url)
	soup = BeautifulSoup(html, 'html.parser')

	for item in soup.find('ol', class_='product-items').find_all('li', class_='product-item'):
		with counter_lock:
			url = item.find('a')['href']
			name = item.find('img')['alt']

			try:
				price = item.find('span', {'data-price-type': 'finalPrice'})['data-price-amount']
			except:
				price = ''
